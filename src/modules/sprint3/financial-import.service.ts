/**
 * Sprint 3 – Financial Import Service
 *
 * Parses CSV/Excel financial sheets and upserts into:
 *   - Loan (disbursement records)
 *   - LoanRepayment (30-day schedule)
 *   - SavingsRecord (individual + group welfare)
 *   - GroupWelfare + GroupWelfareCollection (receivership)
 *
 * Business rules:
 *   - totalRepayable = principal + (principal × 0.06)  [flat 6% interest]
 *   - 30-day repayment skeleton generated if no repayment rows present
 *   - All records are immutable (no deletes, only status corrections)
...
export class FinancialImportService {
  ...
  private readonly FLAT_INTEREST_RATE = 0.06; // 6% flat as per CSV
  ...
  private async upsertLoanDisbursement(
    ...
    // Find or create a default loan product for CSV imports
    ...
    // Check for existing loan (idempotency by member + disbursedDate + principal)
  ...
  private async upsertLoanRepayment(
    ...
    // Resolve member → most recent active loan
    const user = await this.prisma.user.findFirst({
      where: { tenantId, idNumber: row.idNumber },
    ...
    // Upsert repayment (unique on loanId + dayNumber + tenantId)
  ...
  private async upsertSavingsRecord(
    ...
    // Idempotency: check by memberId + weekNumber + tenantId
  ...
  private async upsertGroupWelfareCollection(
    ...
    // Find or create GroupWelfare entity
    ...
    // Upsert collection (unique on groupId + weekNumber + tenantId)
  ...
  private async validateLoanDisbursementRow(
    ...
    if (!idNumber) {
      return { rowNumber, status: 'ERROR', message: 'Missing idNumber', data: raw };
    }
    if (principal <= 0) {
      return { rowNumber, status: 'ERROR', message: 'Invalid principal amount', data: raw };
    }
    ...
    if (!user?.member) {
      return {
        rowNumber,
        status: 'WARNING',
        message: `Member not found for idNumber: ${idNumber}`,
        data: raw,
      };
    }
  ...
  private async validateLoanRepaymentRow(
    ...
    if (dayNumber < 1 || dayNumber > 30) {
      return { rowNumber, status: 'ERROR', message: 'dayNumber must be 1–30', data: raw };
    }
    if (amountPaid <= 0) {
      return { rowNumber, status: 'ERROR', message: 'Invalid amountPaid', data: raw };
    }
    ...
    if (!loan) {
      return {
        rowNumber,
        status: 'WARNING',
        message: `No active loan for idNumber: ${idNumber}`,
        data: raw,
      };
    }
  ...
  private async validateSavingsRow(
    ...
    if (weekNumber < 1 || weekNumber > 52) {
      return { rowNumber, status: 'ERROR', message: 'weekNumber must be 1–52', data: raw };
    }
    if (amount <= 0) {
      return { rowNumber, status: 'ERROR', message: 'Invalid amount', data: raw };
    }
  ...
  private async validateGroupWelfareRow(
    ...
    if (!stageName) {
      return { rowNumber, status: 'ERROR', message: 'Missing stageName', data: raw };
    }
    if (weekNumber < 1 || weekNumber > 52) {
      return { rowNumber, status: 'ERROR', message: 'weekNumber must be 1–52', data: raw };
    }
    if (amountCollected < 0) {
      return { rowNumber, status: 'ERROR', message: 'Invalid amountCollected', data: raw };
    }
  ...
  private extractAmount(sheetType: FinancialSheetType, raw: Record<string, unknown>): number {
    switch (sheetType) {
      case FinancialSheetType.LOAN_DISBURSEMENT:
        return Number(raw['principal'] ?? 0);
      case FinancialSheetType.LOAN_REPAYMENT:
        return Number(raw['amountPaid'] ?? 0);
      case FinancialSheetType.SACCO_SAVINGS:
        return Number(raw['amount'] ?? 0);
      case FinancialSheetType.GROUP_WELFARE:
        return Number(raw['amountCollected'] ?? 0);
      default:
        return 0;
    }
  }
