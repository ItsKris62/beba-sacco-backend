...
import { StkExpiryProcessor } from '../mpesa/jobs/stk-expiry.processor';
import { MpesaB2cTimeoutProcessor } from '../mpesa/processors/mpesa-b2c-timeout.processor';
...
const QUEUE_PROCESSOR_SUPPORT_PROVIDERS: Provider[] = [PlunkService, GuarantorValidationService];
...
/**
 * The web process keeps processors enabled while WORKER_MODE=false so Render can
 * roll out this change before the separate worker service is switched on.
 * Once the worker is active, set WORKER_MODE=true on the web service to stop
 * registering web-side fallback processors.
 */
...
export function getQueueProcessorProviders(options: QueueModuleOptions): Provider[] {
  if (!shouldRegisterQueueProcessors(options)) {
    return [];
  }

  return [...QUEUE_PROCESSOR_SUPPORT_PROVIDERS, ...QUEUE_PROCESSOR_PROVIDERS];
}

function createBullRootModule(): DynamicModule {
...
function createQueueImports(options: QueueModuleOptions): Array<DynamicModule | Type<unknown>> {
  ...
  if (shouldRegisterQueueProcessors(options)) {
    imports.push(...QUEUE_PROCESSOR_IMPORTS);
  }
...
/**
 * Queue Module - BullMQ producers and optional processors.
 *
 * Producers are always registered. Processor registration is mode-aware so the
 * same codebase can run as either the Render web service or the Render worker.
 */
@Module({
  imports: createQueueImports({ mode: 'web' }),
  providers: getQueueProcessorProviders({ mode: 'web' }),
  exports: [BullModule],
})
export class QueueModule {
  static forRoot(options: QueueModuleOptions): DynamicModule {
    return {
      module: QueueModule,
      imports: createQueueImports(options),
      providers: getQueueProcessorProviders(options),
      exports: [BullModule],
    };
  }
}
